...
def detect_columns():
    ...
    try:
        ...
        for col in columns:
            ...
            # Auto-Detection: Object/bool → cat, numerisch mit ≤15 unique → cat
            if is_obj:
                dtypes[col] = "cat"
            elif n_unique <= 15:
                dtypes[col] = "cat"
...
def apply_dictionary():
    ...
    try:
        ...
        ext = full.suffix.lower()
        if ext == ".xlsx":
            dict_df = pd.read_excel(full, engine="openpyxl")
        elif ext == ".xls":
            dict_df = pd.read_excel(full, engine="xlrd")
        elif ext == ".csv":
            dict_df = pd.read_csv(full)
        else:
            return jsonify({"status": "error", "message": f"Nicht unterstütztes Format: {ext}. Erwartet: .xlsx, .xls, .csv"}), 400
                ...
                try:
                    ...
                    if ext2 == ".parquet":
                        ...
                        for col in df_head.columns:
                            n_unique = int(df_head[col].nunique())
                            is_obj = df_head[col].dtype.kind in ("O", "b")
                            data_dtypes[col] = "cat" if (is_obj or n_unique <= 15) else "num"
                    else:
