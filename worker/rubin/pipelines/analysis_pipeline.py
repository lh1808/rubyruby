...
class AnalysisPipeline:
    ...
    def _load_inputs(self) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        if eval_mask is not None:
            assert len(eval_mask) == n, f"eval_mask-Länge ({len(eval_mask)}) ≠ X-Länge ({n}) nach Subsampling."
    ...
    # ------------------------------------------------------------------
    # Explainability (SHAP / Permutation / Segmentanalyse)
    # ------------------------------------------------------------------

    def _run_explainability(self, cfg, X, models, eval_summary, mlflow, report):
        """SHAP/Permutation-Importance und Segmentanalyse für den Champion.

        Nutzt die bestehenden Funktionen aus ``rubin.explainability``.
        Artefakte werden als MLflow-Artefakte geloggt (Plots, CSVs, Segment-Reports).
        """
        shap_cfg = cfg.shap_values
        seg_cfg = cfg.segment_analysis
        ...
        seed = cfg.constants.random_seed
        self._logger.info("Explainability für Champion '%s' (max %d Samples).", champion_name, shap_cfg.n_shap_values)

        # Sampling
        X_expl = X
        if shap_cfg.n_shap_values and len(X) > shap_cfg.n_shap_values:
            rng = np.random.default_rng(seed)
            idx = rng.choice(len(X), size=shap_cfg.n_shap_values, replace=False)
            X_expl = X.iloc[idx].copy()
        ...
        try:
            ...
            try:
                ...
                for plot_name, fig in [
                    ("summary", shap_result.summary_plots),
                    ("average", shap_result.average_plots),
                    ("pdp", shap_result.pdp_plots),
                    ("scatter", shap_result.scatter_plots),
                ]:
                    report.add_explainability_plot(f"SHAP {plot_name.title()}", fig)
                    def _save_plot(p, _fig=fig):
                        _fig.savefig(p, dpi=160, bbox_inches="tight")
                        import matplotlib.pyplot as plt
                        plt.close(_fig)
                    _log_temp_artifact(mlflow, _save_plot, f"SHAP_{plot_name}_{champion_name}.png")
                ...
                def _save_bar(p, _imp=shap_result.importance):
                    save_importance_barplot(_imp, p, top_n=top_n, title=f"SHAP-Importance – {champion_name}")
                _log_temp_artifact(mlflow, _save_bar, f"shap_importance_{champion_name}.png")

                # Importance-Barplot für HTML-Report
                import matplotlib.pyplot as plt
                _imp_plot = shap_result.importance.head(top_n)[::-1]
                ...
                report.add_explainability_plot("SHAP-Importance (Barplot)", _fig_bar)
                plt.close(_fig_bar)

                top_features = list(shap_result.importance.head(seg_cfg.top_n_features).index)
                ...
                top_features = list(imp.head(seg_cfg.top_n_features).index)
        ...
        # ── Segmentanalyse ──
        if seg_cfg.enabled and top_features:
            try:
                from rubin.explainability import build_segment_report, build_feature_segment_report

                seg = build_segment_report(uplift=uplift, quantiles=seg_cfg.quantiles)
                def _save_seg(p, _seg=seg):
                    _seg.to_csv(p)
                _log_temp_artifact(mlflow, _save_seg, f"segment_report_{champion_name}.csv")

                feature_seg = build_feature_segment_report(
                    X=X_expl, uplift=uplift, top_features=top_features,
                    max_features=seg_cfg.top_n_features,
                    max_bins=seg_cfg.max_bins,
                    max_categories=seg_cfg.max_categories,
                )
                def _save_fseg(p, _fs=feature_seg):
                    _fs.to_csv(p)
                _log_temp_artifact(mlflow, _save_fseg, f"feature_segment_report_{champion_name}.csv")

                self._logger.info("Segmentanalyse für '%s' geschrieben (%d Quantile, %d Features).",
                                  champion_name, seg_cfg.quantiles, len(top_features))
            except Exception:
                self._logger.warning("Segmentanalyse fehlgeschlagen.", exc_info=True)
