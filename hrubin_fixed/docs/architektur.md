...
### Schritt 7: Surrogate-Einzelbäume
rubin trainiert bis zu zwei Surrogates pro Analyselauf:
- **CausalForestDML-Surrogate** (`SurrogateTree_CausalForestDML`): Wird immer trainiert, wenn CausalForestDML in den Modellen ist — unabhängig von `surrogate_tree.enabled`. DRTester-Plots werden erzeugt, wenn CausalForestDML auf Rang 1 oder 2 der Champion-Metrik steht (Ranking exkludiert historischen Score und Surrogates).
- **Champion-Surrogate** (`SurrogateTree`): Wird bei `surrogate_tree.enabled: true` trainiert, wenn der Champion nicht CausalForestDML ist. Wenn CausalForestDML selbst Champion ist, wird der CF-Surrogate als Standard-Surrogate referenziert.

Beide nutzen LightGBM oder CatBoost mit `n_estimators=1`. Bei Multi-Treatment wird pro Arm ein separater Baum erzeugt. Die Surrogates konkurrieren nicht um den Champion-Status.

**Target:** Im Cross-Modus (Standard) werden die **Cross-Predicted CATEs** (`Predictions_`-Spalte) als Regressionsziel verwendet — jedes Sample hat einen CATE aus einem Modell, das es nie gesehen hat. Zusätzlich wird eine eigene KFold-CV auf dem Surrogate durchgeführt, damit auch die Surrogate-Metriken ehrlich sind. Für den finalen Export wird der Surrogate auf allen Daten neu trainiert. Im Holdout-Modus werden In-Sample-Predictions auf den Trainingsdaten als Target verwendet (akzeptabel, da die Evaluation auf dem Holdout-Set stattfindet).
