...
from rubin.training import _predict_effect, SURROGATE_MODEL_NAME, SURROGATE_CF_NAME
...
- Die Qualität/Kompatibilität wird über Bundle-Artefakte abgesichert (Preprocessor + Schema)."""
    ...
    @property
    def has_cf_surrogate(self) -> bool:
        """Prüft, ob ein CausalForestDML-Surrogate im Bundle vorhanden ist."""
        return SURROGATE_CF_NAME in self.models
    ...
    def score_surrogate(self, X_raw: pd.DataFrame) -> ProductionOutputs:
        ...
        Bevorzugt den Champion-Surrogate, fällt auf den CF-Surrogate zurück."""
        if self.has_surrogate:
            return self.score(X_raw, model_names=[SURROGATE_MODEL_NAME])
        elif self.has_cf_surrogate:
            return self.score(X_raw, model_names=[SURROGATE_CF_NAME])
        raise ValueError(
            f"Das Bundle enthält keinen Surrogate-Einzelbaum "
            f"(weder '{SURROGATE_MODEL_NAME}' noch '{SURROGATE_CF_NAME}'). "
