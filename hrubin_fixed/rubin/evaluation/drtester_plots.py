    ...
    *,
    ...
    try:
        ...
        try:
            ...
            _logger.debug("DRTester summary() fehlgeschlagen (BLP nicht verfügbar).")
            summary = pd.DataFrame()
        try:
            cal_plot = res.plot_cal(1).get_figure()
            recolor_figure(cal_plot)
        except Exception:
            _logger.debug("DRTester plot_cal fehlgeschlagen.")
        try:
            qini_plot = res.plot_qini(1).get_figure()
            recolor_figure(qini_plot)
        except Exception:
            _logger.debug("DRTester plot_qini fehlgeschlagen.")
        try:
            toc_plot = res.plot_toc(1).get_figure()
            recolor_figure(toc_plot)
        except Exception:
            _logger.debug("DRTester plot_toc fehlgeschlagen.")
        try:
            policy_values = res.get_policy_values(1)
        except Exception:
            _logger.debug("DRTester get_policy_values fehlgeschlagen.")
    except Exception as e:
