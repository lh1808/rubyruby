...
class AnalysisPipeline:
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow, eval_mask=None):
        ...
        try:
            ...
            use_separate_train = holdout_data is not None
            ...
            if not is_mt:
                fitted_tester_bt = fit_drtester_nuisance(
                    ...
                    X_train=X if use_separate_train else X_val,
                    T_train=T if use_separate_train else T_val,
                    Y_train=Y if use_separate_train else Y_val,
                    cv=_drtester_cv,
                )
                ...
                def _fit_arm(arm):
                    ...
                    # Train-Daten: bei External Eval separate Daten, bei Cross-Validation
                    # dieselben wie val (wie in causaluka)
                    if use_separate_train:
                        ...
                        arm_X_train = arm_X_val
                        arm_T_train = arm_T_val
                        arm_Y_train = arm_Y_val
    ...
    def _evaluate_bt(self, cfg, X, T, Y, holdout_data, mname, dfp, y, t, tuned_params_by_model, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        try:
            ...
            # Train-Daten: bei External Eval separate, bei Cross-Validation gleich val
            X_val_dr = holdout_data[0] if holdout_data is not None else X_eval
            T_val_dr = holdout_data[1] if holdout_data is not None else T_eval
            Y_val_dr = holdout_data[2] if holdout_data is not None else Y_eval
            X_train_dr = X if holdout_data is not None else X_eval
            T_train_dr = T if holdout_data is not None else T_eval
            Y_train_dr = Y if holdout_data is not None else Y_eval

            bundle = evaluate_cate_with_plots(
                X_val=X_val_dr, T_val=T_val_dr, Y_val=Y_val_dr,
                cate_preds_val=dfp_eval[pred_col].to_numpy(dtype=float),
                X_train=X_train_dr, T_train=T_train_dr, Y_train=Y_train_dr,
    ...
    def _evaluate_historical_score(self, cfg, X, T, Y, holdout_data, preds, hist_score_eval, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        try:
            ...
            # Train-Daten: bei External Eval separate, sonst gleich val (wie causaluka)
            X_train_hist = X if holdout_data is not None else eval_X
            T_train_hist = T if holdout_data is not None else eval_t
            Y_train_hist = Y if holdout_data is not None else eval_y
            ...
            if ft_hist is not None:
                bundle_h = evaluate_cate_with_plots(
                    ...
                    X_train=X_train_hist, T_train=T_train_hist, Y_train=Y_train_hist,
                ...
                bundle_h = evaluate_cate_with_plots(
                    ...
                    cate_preds_val=hist_score,
                    X_train=X_train_hist, T_train=T_train_hist, Y_train=Y_train_hist,
                    cate_preds_train=hist_score if holdout_data is not None else None,
                    n_groups=10,
                    n_bootstrap=getattr(self, '_eval_n_bootstrap', 1000),
                )
    ...
    def _determine_champion(self, cfg, eval_summary, models):
        ...
        _exclude = {SURROGATE_MODEL_NAME, SURROGATE_CF_NAME}
        entries = [
        ...
        if run_drtester and not is_mt:
            try:
                ...
                if pred_col_s in surrogate_df.columns:
                    ...
                    X_train_surr = X if holdout_data is not None else eval_X
                    T_train_surr = T if holdout_data is not None else t_s
                    Y_train_surr = Y if holdout_data is not None else y_s
                    ...
                    if ft_surr is not None:
                        bundle_s = evaluate_cate_with_plots(
                            ...
                            X_train=X_train_surr, T_train=T_train_surr, Y_train=Y_train_surr,
                            cate_preds_train=cate_vals, n_groups=10,
                            n_bootstrap=getattr(self, '_eval_n_bootstrap', 1000),
                        )
                    else:
                        pj = 1 if cfg.constants.parallel_level <= 1 else -1
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        # Run-Name: "Analyse – <adjektiv>-<nomen>" für Unterscheidbarkeit
                        ...
                        try:
                            ...
                            ranked = sorted(
                                [(n, m.get(metric, 0)) for n, m in eval_summary.items()
                                 if n not in (SURROGATE_MODEL_NAME, SURROGATE_CF_NAME) and n in models and isinstance(m, dict)],
