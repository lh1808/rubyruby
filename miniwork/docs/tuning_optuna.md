...
final_model_tuning:
  ...
  search_space:
    lgbm:
      ...
      num_leaves: {type: "int", low: 4, high: 20}
...
Die FMT-Suchräume sind bewusst konservativer als die BL-Suchräume: höhere `min_child_samples` (100–1000), niedrigere `max_bin` (10–63) und weniger `n_estimators` (50–300) verhindern Overfitting auf die verrauschten DR-Residuen. Bei LightGBM wird automatisch `subsample_freq=1` injiziert, wenn `subsample` im Suchraum ist — das erzwingt Bagging in jedem Boosting-Schritt.
...
Der TPE-Sampler ist mit drei Optimierungen konfiguriert:
