...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    if dpi:
        ...
        if isinstance(target_val, list):
            ...
            h += f'<div class="cd"><div class="cd-l">Zielspalte</div><div class="cd-v">{escape(str(target_val))}</div></div>'
        h += f'<div class="cd"><div class="cd-l">Treatment-Spalte</div><div class="cd-v">{escape(str(dpi.get("treatment", "T")))}</div></div>'
        if dpi.get("score_name"):
            h += f'<div class="cd"><div class="cd-l">Score-Spalte</div><div class="cd-v">{escape(str(dpi["score_name"]))}</div></div>'
    ...
    if cs.get("feature_selection_enabled"):
        ...
        if fs.get("n_before") and fs.get("n_after"):
            ...
            h += f'<div class="cd hl"><div class="cd-l">Features gesamt</div><div class="cd-v">{fs["n_before"]} → {fs["n_after"]} (−{red:.0f}%)</div></div>'
