...
def remove_highly_correlated_features(
    ...
    # Pearson zuerst (linear, schnell), dann Spearman nur auf Überlebenden
    to_drop: set = set()
    ...
    for method in sorted(methods, key=lambda m: 0 if m == "pearson" else 1):
        ...
        new_drops = {col for col in upper.columns if any(upper[col] > threshold)}
        to_drop |= new_drops
    ...
    if to_drop_sorted:
        _logger.info(
            "Korrelationsfilter (|r| > %.2f): %d Features entfernt, %d verbleiben.",
            threshold, len(to_drop_sorted), len(numeric_cols) - len(to_drop_sorted),
        )
    else:
        _logger.info("Korrelationsfilter (|r| > %.2f): keine Features entfernt.", threshold)
    return X.drop(columns=to_drop_sorted, errors="ignore"), to_drop_sorted
