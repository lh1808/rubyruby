...
- **Analyse-Pipeline**: Trainieren, Feature-Selektion (LGBM + GRF Union), Tuning (BL + FMT mit Stability Penalty), Evaluieren, Surrogate-Einzelbäume (CausalForestDML + Champion), HTML-Report mit klickbaren Plots (Lightbox).
- **Production-Pipeline**: Stabiles Scoring auf neuen Daten (inkl. Champion- und CausalForestDML-Surrogate-Option).
