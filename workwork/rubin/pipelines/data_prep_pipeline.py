...
class DataPrepPipeline:
    ...
    def _run_inner(self, dp, out_dir) -> DataPrepOutputs:
        ...
        # object- und category-Spalten: numerische Spalten retten, Rest als kategorisch
        ...
        # Auch explizit als kategorisch gelistete Spalten werden geprüft: Wenn eine
        # Spalte einen numerischen dtype hat (int/float), ist sie nicht kategorisch —
        # unabhängig davon, was der Detect-Endpoint oder die Config sagt.
        _coerced_count = 0
        _reclassified = []
        validated_categorical = []
        for col in X.columns:
            if col in categorical_columns:
                # Explizit gelistet — aber tatsächlich numerisch? Dann reklassifizieren.
                if X[col].dtype.kind in ("i", "u", "f"):
                    _reclassified.append(col)
                    continue  # nicht als kategorisch übernehmen
                elif X[col].dtype == "object":
                    # Object-Spalte aus der Config: Trotzdem prüfen ob numerisch
        ...
        if _reclassified:
            self._logger.info(
                "Typ-Korrektur: %d Spalten waren als kategorisch konfiguriert, "
                "sind aber numerisch: %s",
                len(_reclassified), _reclassified[:10],
            )
        if _coerced_count:
            self._logger.info(
