...
class AnalysisPipeline:
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow, eval_mask=None):
        ...
        try:
            ...
            # Train-Daten NUR bei External Eval: DRTester vergleicht dann Val- vs Train-
            # Kalibrierung (Overfit-Check). Ohne External ist X_train == X_val (gleiche
            # Daten), was zu degenerierten Statistiken führt (0/0 in cal_r_squared,
            # NaN in TOC-Bootstrap) und die gesamte DRTester-Auswertung zerstört.
            use_train_in_drtester = holdout_data is not None
            has_train = False
            if use_train_in_drtester:
                has_train = any(
    ...
    def _evaluate_bt(self, cfg, X, T, Y, holdout_data, mname, dfp, y, t, tuned_params_by_model, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        try:
            train_col = f"Train_{mname}"
            cate_train = None
            # Train-CATEs nur bei External Eval verwenden. Ohne External ist X_train == X_val,
            # und EconML's Cal/Qini/TOC-Vergleich val-vs-train wird degeneriert
            # (dr_train_ ≈ dr_val_ → 0/0 in Kalibrierung → NaN-Kaskade).
        ...
        try:
            ...
            if ft_hist is not None:
                bundle_h = evaluate_cate_with_plots(
                    ...
                    cate_preds_train=hist_score if holdout_data is not None and not use_mask else None,
                    n_groups=10,
                    n_bootstrap=getattr(self, '_eval_n_bootstrap', 1000),
                )
            else:
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        # Falls ja und die Config kein explizites Experiment vorgibt, verwende es,
        # damit DataPrep- und Analyse-Runs im gleichen Experiment landen.
        ...
        try:
            ...
            if dp_exp_file.is_file():
                ...
                if dp_exp and dp_exp == cfg.mlflow.experiment_name:
                    self._logger.info("MLflow-Experiment '%s' (aus DataPrep übernommen).", dp_exp)
                elif dp_exp:
                    self._logger.info(
                        "DataPrep-Experiment '%s' gefunden, Config nutzt '%s'. Verwende Config-Wert.",
