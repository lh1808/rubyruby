...
passen (gleiche Reihenfolge/Länge)."""
    ...
    def evaluate_blp(self, *args, **kwargs):
        """Override: Finale NaN/Inf-Sicherheit vor OLS-Aufruf.
        EconML's evaluate_blp() ruft OLS(self.dr_val_, add_constant(self.cate_preds_val_)).
        Wenn nach allen vorherigen Sanitisierungen noch NaN/Inf überlebt hat,
        fangen wir es hier ab."""
        if self.dr_val_ is not None:
    ...
    def evaluate_all(
        ...
        Wenn einer fehlschlägt (z.B. BLP bei extremen DR-Werten), werden die
        anderen trotzdem ausgeführt — insbesondere Qini/TOC für Policy Values."""
...
def generate_sklift_plots(
    ...
    if _SKLIFT_AVAILABLE:
        try:
            ...
            sk_qini = fig_qini
            _logger.info("sklift plot_qini_curve erfolgreich (generate_sklift_plots)")
        except Exception as e:
    ...
    if _SKLIFT_AVAILABLE:
        try:
            ...
            sk_pct = fig_pct
            _logger.info("sklift plot_uplift_by_percentile erfolgreich (generate_sklift_plots)")
        except Exception as e:
    ...
    if _SKLIFT_AVAILABLE:
        try:
            fig_tb, ax_tb = plt.subplots(figsize=(10, 6))
            plot_treatment_balance_curve(
                uplift=uplift, treatment=t_val_arr, winsize=0.1, ax=ax_tb,
            )
            sk_tb = fig_tb
            _logger.info("sklift plot_treatment_balance_curve erfolgreich (generate_sklift_plots)")
        except Exception as e:
...
def evaluate_cate_with_plots(
    *,
    ...
    # Eingangs-Sanitisierung: CATE-Predictions können NaN/Inf enthalten
    # (z.B. durch instabile model_final Predictions oder Division durch 0 in DR-Outcomes).
    cate_preds_val = np.nan_to_num(np.asarray(cate_preds_val, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
    if cate_preds_train is not None:
        cate_preds_train = np.nan_to_num(np.asarray(cate_preds_train, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if fitted_tester is not None:
        ...
        # fit_on_train nur True setzen wenn auch tatsächlich Train-Predictions
        # vorhanden sind. Ohne cate_preds_train crashen evaluate_cal/evaluate_uplift,
        # weil sie self.cate_preds_train_ subscripten der None ist.
        has_train = cate_preds_train is not None
        tester.fit_on_train = fitted_tester.fit_on_train and has_train
        if has_train and hasattr(fitted_tester, 'dr_train_') and fitted_tester.dr_train_ is not None:
            tester.dr_train_ = CustomDRTester._sanitize_dr(fitted_tester.dr_train_)
        else:
            # Ohne dr_train_ darf fit_on_train nicht True sein, sonst crashen
            # evaluate_cal/evaluate_uplift bei der Train-seitigen Berechnung.
            tester.fit_on_train = False
        # cate_preds_train_ aus dem ARGUMENT setzen, nicht aus fitted_tester.
        # fitted_tester.cate_preds_train_ ist typischerweise None, weil fit_drtester_nuisance
        # nur Nuisance-Modelle fittet und keine echten CATE-Predictions kennt.
        # Die echten Train-CATEs kommen vom jeweiligen Modell als cate_preds_train.
        if cate_preds_train is not None:
    ...
    try:
        ...
        # Jedes Ergebnis einzeln extrahieren — bei Partial-Failure (z.B. BLP crasht
        # aber Qini läuft) sollen die funktionierenden Teile trotzdem genutzt werden.
        try:
            summary = res.summary()
        except Exception as e:
        ...
        try:
            ...
            _logger.warning("DRTester plot_cal fehlgeschlagen: %s", e)
        try:
            qini_plot = res.plot_qini(1).get_figure()
            recolor_figure(qini_plot)
        except Exception as e:
            _logger.warning("DRTester plot_qini fehlgeschlagen: %s", e)
        try:
            toc_plot = res.plot_toc(1).get_figure()
            recolor_figure(toc_plot)
        except Exception as e:
            _logger.warning("DRTester plot_toc fehlgeschlagen: %s", e)
        try:
    ...
    if _SKLIFT_AVAILABLE:
        try:
            ...
            sk_qini = fig_qini
            _logger.info("sklift plot_qini_curve erfolgreich (evaluate_cate_with_plots)")
        except Exception as e:
            _logger.warning("sklift Qini-Curve fehlgeschlagen: %s", e)
    ...
    if _SKLIFT_AVAILABLE:
        try:
            ...
            sk_pct = fig_pct
            _logger.info("sklift plot_uplift_by_percentile erfolgreich (evaluate_cate_with_plots)")
        except Exception as e:
            _logger.warning("sklift Uplift-by-Percentile fehlgeschlagen, nutze Fallback: %s", e)
    if sk_pct is None:
        sk_pct = _native_uplift_by_percentile(y_val_arr, uplift, t_val_arr)
    ...
    if _SKLIFT_AVAILABLE:
        try:
            ...
            sk_tb = fig_tb
            _logger.info("sklift plot_treatment_balance_curve erfolgreich (evaluate_cate_with_plots)")
        except Exception as e:
            _logger.warning("sklift Treatment-Balance fehlgeschlagen, nutze Fallback: %s", e)
    if sk_tb is None:
        sk_tb = _native_treatment_balance(uplift, t_val_arr)
