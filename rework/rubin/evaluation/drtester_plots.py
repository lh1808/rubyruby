...
(z.B. BLP bei extremen DR-Werten), werden die funktionierenden Teile
trotzdem korrekt gespeichert."""

    def __init__(self, *, blp_res=None, cal_res=None, qini_res=None, toc_res=None):
        # EvaluationResults.__init__ erwartet Non-None Sub-Results.
        # Bei Partial-Failure direkt zuweisen statt super().__init__().
        self.blp = blp_res
        self.cal = cal_res
        self.qini = qini_res
        self.toc = toc_res
    ...
    def summary(self, *, tmt=0):
        """Erzeugt eine Summary-Tabelle aus den verfügbaren Sub-Results.

        Die EconML-Basisklasse crasht, wenn einzelne Sub-Results (cal, qini, toc)
        None sind. Diese Überschreibung baut eine partielle Summary aus den
        vorhandenen Ergebnissen zusammen."""
        rows = []
    ...
    def get_policy_values(
        ...
        # Guard: Wenn evaluate_uplift(qini) fehlgeschlagen ist, gibt es keine Kurvendaten.
        if self.qini is None or not hasattr(self.qini, 'curves') or not hasattr(self.qini, 'treatments'):
            return pd.DataFrame()
        if not self.qini.curves or not self.qini.treatments:
            return pd.DataFrame()
        if tmt >= len(self.qini.treatments):
            return pd.DataFrame()

        # Kurvendaten für das gewählte Treatment
        treatment_key = self.qini.treatments[tmt]
        if treatment_key not in self.qini.curves:
            return pd.DataFrame()
        qini_curve = self.qini.curves[treatment_key]
...
passen (gleiche Reihenfolge/Länge)."""
    ...
    def _sanitize_dr(arr: np.ndarray) -> np.ndarray:
        """Ersetzt NaN/Inf in DR-Outcomes und clippt extreme Werte.
        ...
        # Schritt 1: NaN/Inf durch 0 ersetzen
        out = np.nan_to_num(out, nan=0.0, posinf=0.0, neginf=0.0)
        # Schritt 2: Extreme finite Werte clippen (Percentil-basiert)
        finite = out[np.isfinite(out)]
        if len(finite) > 0:
            lo, hi = np.percentile(finite, [0.5, 99.5])
            out = np.clip(out, lo, hi)
        # Schritt 3: Finale Sicherheit — falls noch NaN/Inf überlebt hat
        out = np.nan_to_num(out, nan=0.0, posinf=0.0, neginf=0.0)
        return out

    def evaluate_blp(self, *args, **kwargs):
        """Override: Ruft evaluate_blp mit bereits sanitisierten Daten auf."""
        return super().evaluate_blp(*args, **kwargs)
    ...
    def evaluate_all(
        ...
        Jeder Sub-Test (BLP, Calibration, Qini, TOC) läuft unabhängig.
        Wenn einer fehlschlägt, werden die anderen trotzdem ausgeführt."""
        ...
        # Einmalige Sanitisierung aller Arrays vor den Sub-Tests.
        ...
        # Jeder Sub-Test unabhängig: BLP-Crash darf Qini/TOC nicht blockieren
        blp_res = None
        try:
            blp_res = self.evaluate_blp()
        except Exception as e:
            _logger.warning("evaluate_blp fehlgeschlagen (BLP-Statistik nicht verfügbar): %s", e)

        cal_res = None
        try:
            cal_res = self.evaluate_cal(n_groups=n_groups)
        except Exception as e:
            _logger.warning("evaluate_cal fehlgeschlagen: %s", e)

        qini_res = None
        try:
            qini_res = self.evaluate_uplift(metric="qini", n_bootstrap=n_bootstrap)
        except Exception as e:
            _logger.warning("evaluate_uplift(qini) fehlgeschlagen: %s", e)

        toc_res = None
        try:
            toc_res = self.evaluate_uplift(metric="toc", n_bootstrap=n_bootstrap)
        except Exception as e:
            _logger.warning("evaluate_uplift(toc) fehlgeschlagen: %s", e)

        self.res = CustomEvaluationResults(
            blp_res=blp_res, cal_res=cal_res, qini_res=qini_res, toc_res=toc_res,
        )
        return self.res
...
class DrTesterPlotBundle:
    """Sammelt alle Plots und Tabellen aus einer DRTester + sklift Evaluation.

    Bietet Convenience-Methoden für die drei Schritte nach der Erzeugung:
    MLflow-Logging, HTML-Report-Registrierung, Figure-Freigabe."""

    summary: pd.DataFrame
    cal_plot: Optional[plt.Figure]
    qini_plot: Optional[plt.Figure]
    toc_plot: Optional[plt.Figure]
    ...
    def log_to_mlflow(self, mlflow, label: str, log_temp_artifact_fn=None) -> None:
        ...
        log_temp_artifact_fn: Callable (fn(mlflow, content_fn, filename) → None)
            Pipeline-Hilfsfunktion zum Speichern temporärer Artefakte.
            Wenn None, werden nur Figures geloggt."""
...
def filter_tester_for_mask(
    ...
    Wird bei Train-Many-Evaluate-One benötigt: der Nuisance-Fit wurde auf dem
    gesamten Datensatz durchgeführt, aber die Evaluation soll nur auf den
    Eval-Zeilen laufen. Die DR-Outcomes werden entsprechend gefiltert,
    dr_train_ bleibt ungefiltert (voller Datensatz als Referenz).

    Returns None wenn die Filterung fehlschlägt."""
    try:
        ft = CustomDRTester(
            ...
            cate=None,
            cate_preds_val=np.zeros(n_eval),  # Placeholder, wird in evaluate_cate_with_plots überschrieben
        )
        ...
        ft.Dval = fitted_tester.Dval[eval_mask] if hasattr(fitted_tester, 'Dval') and fitted_tester.Dval is not None else None
        ft.treatments = fitted_tester.treatments
        ft.n_treat = fitted_tester.n_treat
        # dr_train_ bleibt ungefiltert — voller Datensatz als Referenz (wie causaluka)
        ft.dr_train_ = (fitted_tester.dr_train_
                        if hasattr(fitted_tester, 'dr_train_') and fitted_tester.dr_train_ is not None
                        else fitted_tester.dr_val_)
        ft.fit_on_train = True
        return ft
    except Exception:
        _logger.debug("filter_tester_for_mask fehlgeschlagen, Fallback auf None.")
        return None
...
def generate_sklift_plots(
    ...
    # ── Qini-Kurve (plot_qini_curve unterstützt ax=) ──
    if _SKLIFT_AVAILABLE:
        fig_qini = None
        try:
            fig_qini, ax_qini = plt.subplots(figsize=(10, 6))
            plot_qini_curve(
                y_true=y_val_arr, uplift=uplift, treatment=t_val_arr,
                perfect=False, ax=ax_qini,
            )
            sk_qini = fig_qini
        except Exception as e:
            _logger.warning("sklift Qini-Curve fehlgeschlagen: %s", e)
            if fig_qini is not None:
                plt.close(fig_qini)

    # ── Uplift-by-Percentile (kein ax=-Support → Figure aus Rückgabe extrahieren) ──
    if _SKLIFT_AVAILABLE:
        try:
            ...
            _logger.warning("sklift Uplift-by-Percentile fehlgeschlagen, nutze Fallback: %s", e)
    if sk_pct is None:
        sk_pct = _native_uplift_by_percentile(y_val_arr, uplift, t_val_arr)

    # ── Treatment-Balance (kein ax=-Support → Figure aus Rückgabe extrahieren) ──
    if _SKLIFT_AVAILABLE:
        try:
            ...
            _logger.warning("sklift Treatment-Balance fehlgeschlagen, nutze Fallback: %s", e)
    if sk_tb is None:
...
def evaluate_cate_with_plots(
    *,
    ...
    Zwei Modi:
    - fitted_tester gegeben → Nuisance-Ergebnisse wiederverwenden (schnell)
    - fitted_tester=None → Nuisance komplett neu fitten (Fallback)

    In beiden Fällen werden alle DRTester-Tests (BLP, Cal, Qini, TOC)
    und alle sklift-Plots erzeugt. Fehlende cate_preds_train werden
    durch cate_preds_val ersetzt (Cross-Modus: gleiche Daten)."""

    # ── Sanitisierung ──
    cate_preds_val = np.nan_to_num(np.asarray(cate_preds_val, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
    effective_train_preds = cate_preds_train if cate_preds_train is not None else cate_preds_val
    effective_train_preds = np.nan_to_num(np.asarray(effective_train_preds, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)

    # ── Tester vorbereiten ──
    tester = _prepare_tester(
        fitted_tester=fitted_tester,
        model_regression=model_regression,
        model_propensity=model_propensity,
        cate_preds_val=cate_preds_val,
        cate_preds_train=effective_train_preds,
        X_val=X_val, T_val=T_val, Y_val=Y_val,
        X_train=X_train, T_train=T_train, Y_train=Y_train,
    )

    # ── DRTester auswerten + Plots extrahieren ──
    summary, cal_plot, qini_plot, toc_plot, policy_values = _run_drtester(
        tester, X_val, X_train, n_groups, n_bootstrap,
    )

    # ── sklift-Plots (unabhängig vom DRTester) ──
    sk_qini, sk_pct, sk_tb = generate_sklift_plots(cate_preds_val, T_val, Y_val)
...
def _prepare_tester(
    *,
    fitted_tester: Optional[CustomDRTester],
    model_regression: Any,
    model_propensity: Any,
    cate_preds_val: np.ndarray,
    ...
    T_train: Optional[np.ndarray],
    Y_train: Optional[np.ndarray],
) -> CustomDRTester:
    """Baut einen auswertungsbereiten DRTester — entweder aus Pre-Fit oder neu."""

    if fitted_tester is not None:
        # Nuisance wiederverwenden: nur CATE-Predictions austauschen
        tester = CustomDRTester(
            model_regression=getattr(fitted_tester, '_model_regression_ref', None),
            model_propensity=getattr(fitted_tester, '_model_propensity_ref', None),
            cate=None,
    ...
    # Train-Daten: wenn vorhanden nutzen, sonst val als Fallback (causaluka-Muster)
    Xtr = X_train.values if X_train is not None else X_val.values
    Dtr = np.asarray(T_train).ravel() if T_train is not None else np.asarray(T_val).ravel()
    ytr = np.asarray(Y_train).ravel() if Y_train is not None else np.asarray(Y_val).ravel()
    tester.fit_nuisance(
        Xval=X_val.values,
        Dval=np.asarray(T_val).ravel(),
        yval=np.asarray(Y_val).ravel(),
        Xtrain=Xtr, Dtrain=Dtr, ytrain=ytr,
    )
    return tester
...
def _run_drtester(
    ...
    """Führt evaluate_all aus und extrahiert Plots resilient.

    Jeder Plot wird einzeln extrahiert — wenn einer fehlschlägt, laufen
    die anderen trotzdem. Gibt (summary, cal_plot, qini_plot, toc_plot,
    policy_values) zurück."""

    summary, cal_plot, qini_plot, toc_plot, policy_values = (
        pd.DataFrame(), None, None, None, pd.DataFrame(),
    )

    try:
        Xtrain_arr = X_train.values if X_train is not None else None
        res = tester.evaluate_all(X_val.values, Xtrain_arr, n_groups=n_groups, n_bootstrap=n_bootstrap)

        # Summary (unser Override handhabt None-Sub-Results)
        try:
            summary = res.summary()
        except Exception as e:
            _logger.warning("DRTester summary() fehlgeschlagen: %s", e)

        # Plots: (sub_result, plot_method, plot_name) — iterativ statt repetitiv
        plot_specs = [
            (res.cal, "plot_cal", "cal_plot"),
            (res.qini, "plot_qini", "qini_plot"),
            (res.toc, "plot_toc", "toc_plot"),
        ]
        plots = {}
        for sub_result, method_name, plot_name in plot_specs:
            if sub_result is not None:
                try:
                    fig = getattr(res, method_name)(1).get_figure()
                    recolor_figure(fig)
                    plots[plot_name] = fig
                except Exception as e:
                    _logger.warning("DRTester %s fehlgeschlagen: %s", method_name, e)

        cal_plot = plots.get("cal_plot")
        qini_plot = plots.get("qini_plot")
        toc_plot = plots.get("toc_plot")

        # Policy Values (benötigt qini-Ergebnisse)
        if res.qini is not None:
            try:
                policy_values = res.get_policy_values(1)
            except Exception as e:
                _logger.warning("DRTester get_policy_values fehlgeschlagen: %s", e)
    except Exception as e:
    ...
    return summary, cal_plot, qini_plot, toc_plot, policy_values
