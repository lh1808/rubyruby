...
class AnalysisPipeline:
    ...
    def _load_inputs(self) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, Optional[np.ndarray]]:
        """Lädt X, T, Y und optional einen historischen Vergleichsscore."""
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow):
        ...
        for mname, dfp in preds.items():
            ...
            if is_mt:
                ...
                for k in range(1, K):
                    ...
                    arm_T_bin = (eval_T[arm_mask] == arm).astype(int)
                ...
                try:
                    sk_qini, sk_pct, sk_tb = generate_sklift_plots(cate_vals, eval_T, eval_Y)
                    if sk_qini is not None:
    ...
    def _evaluate_bt(self, cfg, X, T, Y, holdout_data, mname, dfp, y, t, tuned_params_by_model, eval_summary, policy_values_dict, mlflow, fitted_tester=None):
        ...
        try:
            ...
            bundle = evaluate_cate_with_plots(
                ...
                T_val=(holdout_data[1] if holdout_data is not None else T),
                Y_val=(holdout_data[2] if holdout_data is not None else Y),
                cate_preds_val=dfp[pred_col].to_numpy(dtype=float),
    ...
    def _evaluate_historical_score(self, cfg, X, T, Y, holdout_data, preds, hist_score_eval, eval_summary, policy_values_dict, mlflow, fitted_tester=None):
        ...
        try:
            eval_X = holdout_data[0] if holdout_data is not None else X
            eval_T_val = holdout_data[1] if holdout_data is not None else T
            eval_Y_val = holdout_data[2] if holdout_data is not None else Y

            if fitted_tester is not None:
                # Pre-fitted Nuisance wiederverwenden — schneller und konsistenter
            ...
            for mname, dfp in preds.items():
                ...
                if pred_col not in dfp.columns:
                    continue
                df_cmp = pd.DataFrame({
                    "Y": dfp["Y"].to_numpy(), "T": dfp["T"].to_numpy(),
                    mname: dfp[pred_col].to_numpy(dtype=float), hist_name: hist_score,
                })
    ...
    def _train_and_evaluate_surrogate(self, cfg, X, T, Y, champion_name, preds, holdout_data, models, eval_summary, mlflow):
        """Trainiert den Surrogate-Einzelbaum und evaluiert ihn.

        Ablauf
        ------
        1. CATE-Vorhersagen des Champions als Regressionsziel extrahieren.
        2. Surrogate-Tree trainieren und eigene (Cross-)Predictions erzeugen.
           Bei Multi-Treatment wird pro Treatment-Arm ein separater Baum trainiert,
           da LightGBM/CatBoost nur 1D-Targets unterstützen.
        3. Evaluation mit denselben Metriken wie die CATE-Modelle.

        Returns
        -------
        surrogate_wrapper : SurrogateTreeWrapper
            Der trainierte Wrapper (auf vollen Trainingsdaten gefittet).
        surrogate_df : pd.DataFrame
            Prediction-DataFrame im gleichen Format wie bei CATE-Modellen.
        """
        ...
        seed = cfg.constants.random_seed
        n_splits = cfg.data_processing.cross_validation_splits
        sname = SURROGATE_MODEL_NAME
        is_mt = is_multi_treatment(T)

        champion_df = preds[champion_name]

        if holdout_data is None:
            # --- Cross-Modus ---
            if is_mt:
                mt_cols = sorted([c for c in champion_df.columns if c.startswith(f"Predictions_{champion_name}_T")])
                ...
                target = champion_df[f"Predictions_{champion_name}"].to_numpy()
        ...
        surrogate_wrapper = SurrogateTreeWrapper(
            tree=final_tree, trees=final_trees, champion_name=champion_name,
        )

        # Evaluation mit denselben Metriken
        try:
            y_s = surrogate_df["Y"].to_numpy()
            t_s = surrogate_df["T"].to_numpy()
            if is_mt:
                mt_cols_s = [c for c in surrogate_df.columns if c.startswith(f"Predictions_{sname}_T")]
                scores_2d = surrogate_df[mt_cols_s].to_numpy()
                eval_summary[sname] = mt_eval_summary(y=y_s, t=t_s, scores_2d=scores_2d)
            else:
                s = surrogate_df[f"Predictions_{sname}"].to_numpy()
            ...
            if is_mt:
                ...
                for k in range(1, K):
                    if k - 1 >= len(mt_cols_s):
                        continue
                    arm_scores = surrogate_df[mt_cols_s[k - 1]].to_numpy(dtype=float)
                ...
                pred_col_s = f"Predictions_{sname}"
                if pred_col_s in surrogate_df.columns:
                    cate_vals = surrogate_df[pred_col_s].to_numpy(dtype=float)
                    try:
        ...
        try:
            ...
            mlflow.log_param("surrogate_tree_champion", champion_name)
        except Exception:
            pass
    ...
    def _run_bundle_export(self, cfg, models, eval_summary, X, T, Y, X_full, T_full, Y_full, selected_feature_columns, holdout_data, export_bundle, bundle_dir, bundle_id, mlflow):
        ...
        cate_model_names = [n for n in models.keys() if n != SURROGATE_MODEL_NAME]
        entries = [
        ...
        models_to_export = {n: m for n, m in models.items() if n != SURROGATE_MODEL_NAME}
        ...
        bundler.write_metadata(paths, {
            "models": list(models_to_export.keys()) + ([SURROGATE_MODEL_NAME] if surrogate_exported else []),
            "champion": champion,
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        with mlflow.start_run():
            ...
            if use_external:
                ...
                if cfg.data_files.eval_s_file:
                    try:
                        ...
                        S_eval = self._read_table(cfg.data_files.eval_s_file)[col].to_numpy()
                    except Exception:
                        S_eval = None
            ...
            with patch_categorical_features(X, base_learner_type=cfg.base_learner.type) as cat_indices:
                ...
                eval_summary: Dict[str, Dict[str, float]] = {}
                try:
                    eval_summary, _ = self._run_evaluation(cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow)
                except Exception:
                    self._logger.warning("Uplift-Evaluation fehlgeschlagen.", exc_info=True)

                # Surrogate-Einzelbaum: Champion nachlernen + evaluieren
                if cfg.surrogate_tree.enabled and eval_summary:
            ...
            try:
                summary_path = os.path.join(cfg.optional_output.output_dir or ".", "uplift_eval_summary.json")
            ...
            try:
                ...
                report_dir = cfg.optional_output.output_dir or "."
