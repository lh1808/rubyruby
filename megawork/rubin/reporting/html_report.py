...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    if cs.get("feature_selection_enabled"):
        ...
        h = '<p class="expl">Union-Methode: Top-X% aus jeder Importance-Methode werden vereinigt. Anschließend werden hoch-korrelierte Features entfernt.</p><div class="cg">'
        ...
        n_after_imp = fs.get("n_after_importance")
        if n_imp > 0:
            h += f'<div class="cd"><div class="cd-l">Importance</div><div class="cd-v">{n_imp} entfernt'
            if n_after_imp is not None:
                h += f'<div style="font-size:11px;color:var(--text-l);margin-top:2px">{fs.get("n_before","?")} → {n_after_imp}</div>'
            h += '</div></div>'
        if n_corr > 0:
            h += f'<div class="cd"><div class="cd-l">Korrelation</div><div class="cd-v">{n_corr} entfernt'
            if n_after_imp is not None:
                h += f'<div style="font-size:11px;color:var(--text-l);margin-top:2px">{n_after_imp} → {fs.get("n_after","?")}</div>'
            h += '</div></div>'
        h += '</div>'
