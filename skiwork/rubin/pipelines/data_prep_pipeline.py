...
class DataPrepPipeline:
                ...
                for s in stats:
                    ...
                    if s["treat_rate"] > target_rate and n_control > 0:
                        ...
                        if target_n_treat < n_treat:
                            treat_sample = part[t_mask].sample(n=target_n_treat, random_state=42)
                        ...
                        if target_n_control < n_control:
                            ctrl_sample = part[c_mask].sample(n=target_n_control, random_state=42)
    ...
    def run(self) -> DataPrepOutputs:
        ...
        if dp.log_to_mlflow:
            ...
            # Run-Name: "Datenaufbereitung – <adjektiv>-<nomen>" für bessere Unterscheidbarkeit
            from rubin.utils.run_names import generate_run_name
            run_name = generate_run_name("Datenaufbereitung")
