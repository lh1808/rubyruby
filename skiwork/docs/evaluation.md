...
Alle drei Funktionen werden mit dem sicheren `ax=`-Pattern aufgerufen: Eine eigene `Figure` + `Axes` wird vorab erstellt und als Parameter übergeben. Damit ist der Rückgabetyp der sklift-Funktionen irrelevant — die Figure ist immer die kontrollierte Instanz. Dieses Pattern wird konsistent an allen sechs Aufrufstellen (je drei in `generate_sklift_plots` und `evaluate_cate_with_plots`) verwendet und entspricht dem Muster, das auch `compute_qini_curve` intern nutzt.

Bei Nicht-Verfügbarkeit oder Fehlern von sklift (z. B. numpy-Inkompatibilität ≥1.24) werden automatisch native Fallback-Implementierungen verwendet (`_native_uplift_by_percentile`, `_native_treatment_balance`). Jeder Fehler wird ins Log geschrieben (`_logger.warning`), jeder Erfolg bestätigt (`_logger.info`). Nach der Erzeugung wird `recolor_figure()` auf alle Plots angewendet, um das rubin-Farbschema zu übertragen.
