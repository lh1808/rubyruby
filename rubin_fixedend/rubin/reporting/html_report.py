...
class ReportCollector:
    ...
    def add_config(self, cfg) -> None:
        ...
        self.config_summary = {
            ...
            "test_size": cfg.data_processing.test_size,
...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    h = '<div class="cg">'
    for lbl, val in [
        ("Experiment", cs.get("experiment_name", "–")),
    ...
    html = _HTML_TEMPLATE.replace("{{NAV}}", nav_html).replace("{{BODY}}", body_html).replace("{{TIMESTAMP}}", ts).replace("{{CHAMPION}}", escape(champ))
...
.plot-card img{max-width:100%;height:auto;border-radius:6px;margin-top:6px;display:block}
