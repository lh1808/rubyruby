...
def train_and_crosspredict_bt_bo(
    ...
    keep_last_fold_model: bool = False,
) -> pd.DataFrame | tuple[pd.DataFrame, Any, np.ndarray | None]:
  ...
  OptimalTreatment_<model_name>, optional Train_<model_name>_T1, ..."""
    ...
    if n_parallel >= 2 and len(folds) >= 2:
        ...
        try:
            ...
            # Letzten Fold nachträglich für Explainability behalten
            if keep_last_fold_model:
                last_tr, last_va = folds[-1]
                last_fold_model = copy.deepcopy(model)
                last_fold_model.fit(Y[last_tr], T[last_tr], X=X.iloc[last_tr])
                last_fold_val_idx = last_va
    ...
    if n_nan > 0:
        _logger.warning(
            "%s: %d/%d Samples haben NaN Cross-Predictions (fehlende Fold-Zuweisung?). "
            "NaN wird durch 0 ersetzt.",
            model_name, n_nan, len(preds),
        )
