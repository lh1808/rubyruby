...
with patch_categorical_features(X_all, base_learner_type="lgbm"):
    importances = compute_importances(...)  # LightGBM nutzt native kat. Splits
    X_reduced, removed_corr, absorbed_by = remove_highly_correlated_features(X_all, importances=importances)
    # Importance-Umverteilung: entfernte Features → Partner
    X_reduced, removed_imp, _ = select_features_by_importance(X_reduced, importances, ...)
