...
def _default_search_space(base_type: str) -> Dict[str, SearchSpaceParameterConfig]:
    ...
    if base_type == "lgbm":
        return {
            "n_estimators": SearchSpaceParameterConfig(type="int", low=200, high=1000),
    ...
    if base_type == "catboost":
        return {
            "iterations": SearchSpaceParameterConfig(type="int", low=200, high=1500),
            "learning_rate": SearchSpaceParameterConfig(type="float", low=1e-2, high=1.5e-1, log=True),
...
def _default_fmt_search_space(base_type: str) -> Dict[str, SearchSpaceParameterConfig]:
    ...
    if base_type == "lgbm":
        return {
            ...
            "min_child_samples": SearchSpaceParameterConfig(type="int", low=50, high=500),
    ...
    if base_type == "catboost":
        return {
            ...
            "min_data_in_leaf": SearchSpaceParameterConfig(type="int", low=50, high=500),
...
persistiert und in weiteren Folds wiederverwendet."""
    ...
    def tune_final_model(
        ...
        fmt_fixed_params: Für model_final (final_model_tuning.fixed_params)."""
