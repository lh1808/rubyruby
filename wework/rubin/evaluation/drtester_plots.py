...
passen (gleiche Reihenfolge/Länge)."""
    ...
    def evaluate_all(
        ...
        if self.fit_on_train:
            # Calibration, Qini und TOC benötigen Train-Daten (dr_train_).
            # Im Cross-Modus (ohne External Eval) sind keine Train-Daten vorhanden —
            # diese Tests werden dann übersprungen (kein Fehler, nur BLP verfügbar).
            try:
            ...
            _logger.debug("DRTester: Kein Train-Datensatz — Calibration, Qini, TOC werden übersprungen (nur BLP verfügbar).")
...
def evaluate_cate_with_plots(
    *,
    ...
    if fitted_tester is not None:
        ...
        # DR-Outcomes aus dem Pre-Fit übernehmen (bereits sanitisiert in fit_nuisance)
    ...
    try:
        ...
        # Jedes Ergebnis einzeln extrahieren — nur wenn der Sub-Test erfolgreich war.
        # Im Cross-Modus (ohne Train-Daten) sind cal/qini/toc nicht verfügbar.
        if res.blp is not None:
            try:
                summary = res.summary()
            except Exception as e:
